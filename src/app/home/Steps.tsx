...
export const Steps = () => {
  return (
    <section className="mx-auto mt-8 rounded-2xl bg-card px-8 pb-12 pt-10 lg:mt-2 border border-border-color">
      <h1 className="text-center text-3xl font-bold text-primary-fg">3 Simple Steps</h1>
        ...
        <dl className="flex flex-col gap-y-10 lg:flex-row lg:justify-center lg:gap-x-20">
          {STEPS.map(({ title, text }, idx) => (
            <div className="relative self-start pl-14" key={idx}>
              <dt className="text-lg font-bold text-primary-fg">
              ...
              <dd className="text-secondary-fg">{text}</dd>
            </div>
          ))}
        </dl>
      </div>
