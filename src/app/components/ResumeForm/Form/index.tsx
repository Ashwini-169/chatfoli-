...
export const BaseForm = ({
  ...
  <section
    className={`flex flex-col gap-3 rounded-md bg-card p-6 pt-4 shadow transition-opacity duration-200 ${className}`}
  >
    {children}
  </section>
);
    ...
    >
      <div className="flex items-center justify-between gap-4">
        <div className="flex grow items-center gap-2">
          <Icon className="h-6 w-6 text-secondary-fg" aria-hidden="true" />
          <input
            type="text"
            className="block w-full border-b border-transparent text-lg font-semibold tracking-wide text-primary-fg outline-none hover:border-gray-300 hover:shadow-sm focus:border-gray-300 focus:shadow-sm bg-card"
            value={heading}
      ...
      {showForm && addButtonText && (
        <div className="mt-2 flex justify-end">
          <button
            ...
            className="flex items-center rounded-md bg-card py-2 pl-3 pr-4 text-sm font-semibold text-primary-fg shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-input"
          >
            <PlusSmallIcon
              className="-ml-0.5 mr-1.5 h-5 w-5 text-tertiary-fg"
              aria-hidden="true"
            />
            {addButtonText}
          </button>
