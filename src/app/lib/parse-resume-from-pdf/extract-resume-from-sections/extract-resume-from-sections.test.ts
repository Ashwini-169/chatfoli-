...
describe("extract-profile tests - ", () => {
  ...
  it("Email", () => {
    expect(matchEmail(makeTextItem("  hello@chatfolio.org  "))![0]).toBe(
      "hello@chatfolio.org"
    );
  });
  ...
  it("Url", () => {
    expect(matchUrl(makeTextItem("  linkedin.com/in/chatfolio  "))![0]).toBe(
      "linkedin.com/in/chatfolio"
    );
    expect(matchUrl(makeTextItem("hello@chatfolio.org"))).toBeFalsy();
  });
});
